        ...
        return new OpenAPI()
                .info(new Info()
                        ...
                        .description("""
                                 ...
                                 ## 素材上传/落库典型流程（Media Library）
                                 1) `POST /api/upload`（Better Upload）获取预签名 URL；
                                 2) 客户端直传到对象存储（S3）；
                                 3) `POST /api/v1/media-library/batch-finalize` 批量落库生成素材记录；
                                 4) 可选：上传前调用 `POST /api/v1/media-library/duplicate-check` 进行秒传检查。
                ...
                .servers(List.of(
                         ...
                         new Server().url("http://localhost:8083").description("Core Service 本地调试端口（不建议前端直连）")))
