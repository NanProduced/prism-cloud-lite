package nan.produced.prism.device.application.domain.websocket;
...
@Slf4j
@Getter
public class WsMessageProcessingContext {

    /**
     * 终端连接对象
     */
    private final DeviceWsConnection connection;
     ...
     * 私有构造函数，强制使用工厂方法创建
     */
    private WsMessageProcessingContext(DeviceWsConnection connection, String rawMessage) {
     ...
     */
    public static WsMessageProcessingContext create(DeviceWsConnection connection, String rawMessage) {
        if (connection == null) {
            log.warn("WsMessageProcessingContext - 创建websocket消息处理上下文失败：终端连接对象为空");
            return null;
        }

        return new WsMessageProcessingContext(connection, rawMessage);
    }
