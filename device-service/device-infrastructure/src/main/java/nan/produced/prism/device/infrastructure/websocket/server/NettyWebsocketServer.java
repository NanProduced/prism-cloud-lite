package nan.produced.prism.device.infrastructure.websocket.server;
...
import nan.produced.prism.device.infrastructure.websocket.channel.WebsocketChannelInitializer;
...
// 检查是否启用WebSocket服务
        if (!nettyWebsocketProps.isEnabled()) {
        ...
        try {
            ...
            log.info("NettyWebsocketServer - WebSocket服务器启动成功，监听端口: {}",
                    nettyWebsocketProps.getServer().getPort());
        } catch (Exception e) {
            log.error("NettyWebsocketServer - WebSocket服务器启动失败", e);
            // 启动失败直接抛异常，阻止Spring启动
            throw new TechException(TechErrorCode.NETTY_START_ERROR, "WebSocket服务器启动失败，应用无法正常工作", e);
        }
    }
    ...
    private void startWebsocketServerSync() throws Exception {
        ...
        ChannelFuture channelFuture = bootstrap.bind(
                nettyWebsocketProps.getServer().getHost(),
                nettyWebsocketProps.getServer().getPort()
        );
        ...
        if (!channelFuture.await(30, TimeUnit.SECONDS)) {
            shutdownGracefully();
            throw new TechException(TechErrorCode.NETTY_START_ERROR, "WebSocket服务器绑定超时(30秒)");
        }

        if (!channelFuture.isSuccess()) {
            shutdownGracefully();
            throw new TechException(TechErrorCode.NETTY_START_ERROR, "WebSocket服务器绑定失败", channelFuture.cause());
        }
    ...
    private void initializeEventLoopGroups() {
        int bossThreads = nettyWebsocketProps.getServer().getBossThreads();
        bossGroup = new NioEventLoopGroup(bossThreads);
        log.debug("NettyWebsocketServer - Boss线程组初始化完成，线程数: {}", bossThreads);

        int workerThreads = nettyWebsocketProps.getServer().getWorkerThreads();
        if (workerThreads <= 0) {
    ...
    private void configureServerOptions(ServerBootstrap bootstrap) {
        NettyWebsocketProps.Server serverConfig = nettyWebsocketProps.getServer();
