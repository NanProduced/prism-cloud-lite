...
import nan.produced.prism.device.infrastructure.cache.redis.config.RedisKeyConstant;
...
import static nan.produced.prism.device.infrastructure.cache.redis.config.RedisKeyConstant.DEVICE_STATUS_INDEX_KEY;
...
public class DeviceOnlineStatusRedisService implements DeviceOnlineStatusPort {
    ...
    private static final GenericToStringSerializer<Long> LONG_SERIALIZER = new GenericToStringSerializer<>(Long.class);
    ...
    /**
     * 获取状态默认TTL配置 - 用于正常在线状态
     */
    private Duration getDefaultCacheTtl() {
        return Duration.ofSeconds(deviceConfigPort.getDeviceConfig().getOnlineStatus().getDefaultCacheTtl());
    }

    /**
     * 获取重连窗口TTL配置 - 用于离线后等待重连
     */
    private Duration getReconnectCacheTtl() {
        return Duration.ofSeconds(deviceConfigPort.getDeviceConfig().getOnlineStatus().getReconnectCacheTtl());
    }

    /**
     * 获取设备状态Redis Key
     */
    private String getOnlineStatusCacheKey(Long deviceId) {
        return String.format(RedisKeyConstant.DEVICE_STATUS_KEY, deviceId);
    }


    @Override
    public void determinedOps(DeviceOnlineStatus status) {
     ...
     */
    @Override
    public void batchDeterminedOps(List<DeviceOnlineStatus> statusList) {
        ...
        try {
                ...
                public <K, V> Object execute(RedisOperations<K, V> operations) throws DataAccessException {
                    for (DeviceOnlineStatus status : statusList) {
                        ...
                        if (currentStatus == null || currentStatus == OnlineStatus.ONLINE) {
                            performUpdateInPipeline(operations, getOnlineStatusCacheKey(status.getDeviceId()), status);
                        }
                    }
            ...
            log.error("DeviceOnlineStatus - 批量判定设备状态失败: statusList.size={}", statusList.size(), e);
        }
    ...
    public void saveDeviceStatus(DeviceOnlineStatus status) {
        String statusKey = getOnlineStatusCacheKey(status.getDeviceId());
        ...
        try {
            ...
            List<Object> results = (List<Object>) redisTemplate.execute(new SessionCallback<Object>() {
                ...
                public Object execute(RedisOperations operations) throws DataAccessException {
                    ...
                    operations.expire(statusKey, getDefaultCacheTtl());
            ...
            if (results.isEmpty()) {
                ...
                throw new TechException(TechErrorCode.REDIS_TRANSACTION_FAILED,
    ...
    public void removeDeviceStatusForStartupCleanup(Long deviceId) {
        String statusKey = getOnlineStatusCacheKey(deviceId);
    ...
    public List<DeviceOnlineStatus> batchMarkOfflineAndResetTtl(List<Long> deviceIds) {
        ...
        try {
            ...
            Duration reconnectTtl = getReconnectCacheTtl();
    ...
    private void streamAllDeviceIds(LongConsumer consumer) {
        ...
        try {
            DomainDeviceProps.StreamQuery streamQuery = deviceConfigPort.getDeviceConfig().getOnlineStatus().getStreamQuery();
            int pageSize = streamQuery.getPageSize();
            int maxIterations = streamQuery.getMaxIterations();
            long timeoutMs = streamQuery.getTimeoutMs();
    ...
    private List<Long> findExpiredDevicesWithStream(long expireThreshold) {
        try {
            ...
            int batchSize = Math.min(deviceConfigPort.getDeviceConfig().getOnlineStatus().getStreamQuery().getPageSize(), 100);
