package nan.produced.prism.device.infrastructure.websocket.channel;
...
import nan.produced.prism.device.infrastructure.websocket.handler.NettyWebsocketAuthHandler;
import nan.produced.prism.device.infrastructure.websocket.handler.NettyWebsocketFrameHandler;
import org.springframework.stereotype.Component;

import java.util.concurrent.TimeUnit;

/**
 * Netty WebSocket通道初始化器
 *
 * @author Nan
 */
@Slf4j
...
public class WebsocketChannelInitializer extends ChannelInitializer<SocketChannel> {
    ...
    private final NettyWebsocketProps nettyWebsocketProps;
    ...
    protected void initChannel(SocketChannel socketChannel) throws Exception {
        ...
        // 2. HTTP消息聚合器
        pipeline.addLast("http-aggregator", new HttpObjectAggregator(nettyWebsocketProps.getMessage().getMaxAggregatedFrameSize()));

        // 3. Websocket认证处理器（协议升级前将认证信息放入上下文）
        pipeline.addLast("websocket-auth", nettyWebsocketAuthHandler);

        // 4. 如果开启压缩则添加压缩处理
        if (nettyWebsocketProps.getMessage().isCompressionEnabled()) {
        ...
        pipeline.addLast("websocket-protocol", new WebSocketServerProtocolHandler(
                nettyWebsocketProps.getServer().getWebsocketPath(),
                null, // 不限制子协议
                true, // 允许扩展
                nettyWebsocketProps.getMessage().getMaxFrameSize()
        ));

        // 6. 心跳超时检测 - 根据设备55秒心跳要求配置
        if (nettyWebsocketProps.getHeartbeat().isEnabled()) {
            pipeline.addLast("idle-state-handler", new IdleStateHandler(
                    nettyWebsocketProps.getHeartbeat().getReadTimeout(),
                    nettyWebsocketProps.getHeartbeat().getWriteTimeout(),
                    nettyWebsocketProps.getHeartbeat().getAllIdleTimeout(),
                    TimeUnit.SECONDS
            ));
        }
