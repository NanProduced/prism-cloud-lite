package nan.produced.prism.device.infrastructure.websocket.processor;
...
public class ProtocolProcessorRegistry implements WsProtocolProcessorPort {

    /**
     * 协议版本到处理器的映射
     */
    private final Map<ProtocolVersion, WsProtocolMessageProcessor> processorMap = new EnumMap<>(ProtocolVersion.class);

    /**
     * 允许的处理器包名白名单 - 反射安全检查
     */
    private static final Set<String> ALLOWED_PROCESSOR_PACKAGES = Set.of(
            "nan.produced.prism.device.infrastructure.websocket.processor.v10",
            "nan.produced.prism.device.infrastructure.websocket.processor.v11"
    );

    private final ApplicationContext applicationContext;

    private final PrismWebsocketConfigProps websocketConfig;
    ...
    public void initializeProcessors() {
        ...
        for (ProtocolVersion version : ProtocolVersion.values()) {
            ...
            boolean supported = websocketConfig.getProtocol().isVersionSupported(version.getVersion(), version.isSupported());
            ...
            try {
                ...
                // 从Spring容器中获取处理器bean
                WsProtocolMessageProcessor processor = (WsProtocolMessageProcessor) applicationContext.getBean(processorClass);
     ...
     */
    @Override
    public WsProtocolMessageProcessor getProcessor(ProtocolVersion version) {
        if (version == null) {
            throw new IllegalArgumentException("协议版本不能为空");
        }

        WsProtocolMessageProcessor processor = processorMap.get(version);
        if (processor == null) {
