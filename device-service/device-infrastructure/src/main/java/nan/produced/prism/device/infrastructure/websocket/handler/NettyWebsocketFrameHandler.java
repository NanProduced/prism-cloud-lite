...
import nan.produced.prism.device.application.handler.WebsocketMsgMetricsHelper;
...
public class NettyWebsocketFrameHandler extends SimpleChannelInboundHandler<WebSocketFrame> {
    ...
    public NettyWebsocketFrameHandler(WebsocketMessageUseCase websocketMessageUseCase,
                                      WsConnectionManagerPort connectionManagerPort,
    ...
    private void initializeWebsocketSessionAsync(ChannelHandlerContext ctx) {
        try {
            ...
            DevicePrincipal principal = ctx.channel().attr(NettyWebsocketAuthHandler.DEVICE_PRINCIPAL).get();
            ...
            // 4. 创建技术会话对象
            DeviceWsSession technicalSession = DeviceWsSession.builder()
            ...
            DeviceWsConnection connection = websocketMessageUseCase.handleConnectionEstablished(
    ...
    private void handleTextFrame(Long deviceId, String message) {
        ...
        DeviceWsConnection connection = getConnectionByDeviceId(deviceId);
        if (connection == null) {
            log.warn("NettyWebsocketFrameHandler - 连接不存在，无法处理消息: deviceId={}", deviceId);
            return;
        }
        // 创建上下文
        WsMessageProcessingContext context = WsMessageProcessingContext.create(connection, message);
        if (!context.isValid()) {
     ...
     * 根据设备ID获取连接对象
     */
    private DeviceWsConnection getConnectionByDeviceId(Long deviceId) {
        DeviceWsConnection connection = connectionManagerPort.getConnection(deviceId).orElse(null);
