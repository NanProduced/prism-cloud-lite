package nan.produced.prism.device.infrastructure.websocket.handler;
...
public class NettyWebsocketAuthHandler extends ChannelInboundHandlerAdapter {
    ...
    private final NettyWebsocketProps nettyProps;

    private final PrismWebsocketConfigProps prismWsProps;

    private final Executor websocketConnectionExecutor;

    public NettyWebsocketAuthHandler(DeviceAuthUseCase deviceAuthUseCase,
                                     NettyWebsocketProps nettyProps,
                                     PrismWebsocketConfigProps prismWsProps,
                                     @Qualifier("websocketConnectionExecutor") Executor websocketConnectionExecutor) {
        this.deviceAuthUseCase = deviceAuthUseCase;
        this.nettyProps = nettyProps;
        this.prismWsProps = prismWsProps;
        this.websocketConnectionExecutor = websocketConnectionExecutor;
    }

    public static final AttributeKey<DevicePrincipal> DEVICE_PRINCIPAL = AttributeKey.valueOf("devicePrincipal");
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        boolean versionEnabled = prismWsProps.getProtocol()
        ...
        try {
            websocketConnectionExecutor.execute(() -> handleWebSocketAuthentication(ctx, request));
    ...
    private void handleWebSocketAuthentication(ChannelHandlerContext ctx, FullHttpRequest request) {
        try {
            ...
            request.setUri(nettyProps.getServer().getWebsocketPath());
    ...
    private boolean authenticateRequest(ChannelHandlerContext context, FullHttpRequest request) {
        try {
            ...
            String username = params.getOrDefault("username", List.of("")).getFirst();
            String password = params.getOrDefault("password", List.of("")).getFirst();
            String versionStr = params.getOrDefault("protocol_version", List.of("")).getFirst();
            ...
            AuthResult authResult = deviceAuthUseCase.authenticate(new AuthRequest(username, password));
            if (authResult.isSuccess()) {
                DevicePrincipal devicePrincipal = new DevicePrincipal(
                        authResult.getDeviceId(), DeviceAccountStatus.ENABLE);
                context.channel().attr(DEVICE_PRINCIPAL).set(devicePrincipal);
                context.channel().attr(DEVICE_ID).set(devicePrincipal.getDeviceId().toString());
    ...
    private void sendUnsupportedProtocolResponse(ChannelHandlerContext ctx) {
        sendResponse(ctx, HttpResponseStatus.UPGRADE_REQUIRED, "Unsupported versions of the (Prism) WebSocket protocol");
    }

    private void sendErrorResponse(ChannelHandlerContext ctx) {
        sendResponse(ctx, HttpResponseStatus.UNAUTHORIZED, "Authentication failed");
    }

    private void sendServiceUnavailableResponse(ChannelHandlerContext ctx) {
        sendResponse(ctx, HttpResponseStatus.SERVICE_UNAVAILABLE, "The server is overloaded, please try again later");
    }
