package nan.produced.prism.device.boot.security.filter;
 ...
 */
@Slf4j
public class DeviceBasicAuthFilter extends BasicAuthenticationFilter {

    public DeviceBasicAuthFilter(AuthenticationManager authenticationManager) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        try {
            ...
            throw new DeviceResponseException(BusinessErrorCode.AUTHENTICATION_FAILED);
        }
    ...
    private UsernamePasswordAuthenticationToken parseAuthenticationHeader(String header) {
        try {
            ...
            if (delim == -1) {
                log.warn("解析Basic-Auth请求头：Basic Auth格式错误：缺少冒号分隔符");
                return null;
            }
            ...
            if (StringUtils.isBlank(account) || StringUtils.isBlank(password)) {
                log.warn("解析Basic-Auth请求头：Basic Auth格式错误：设备ID或密码为空");
                return null;
            }
            ...
            log.warn("解析Basic-Auth请求头：Basic Auth解码失败", e);
            return null;
        } catch (Exception e) {
